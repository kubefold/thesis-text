...
\hline
\multicolumn{1}{|c|}{\textbf{Phase}} & \multicolumn{1}{c|}{\textbf{Component}} & \multicolumn{1}{>{\centering\arraybackslash}p{2.2cm}|}{\textbf{Hardware Req.}} & \multicolumn{1}{c|}{\textbf{Pods}} & \multicolumn{1}{>{\centering\arraybackslash}p{2.8cm}|}{\textbf{Volume Access}} \\
\hline
\textbf{Setup} & 
PVC & 
Storage & 
...
\begin{enumerate}
    \item Creates a new \texttt{PersistentVolumeClaim} and sets the \texttt{storageClassName} parameter,
    \item When the \texttt{PersistentVolumeClaim} is bound to a specific volume, it creates several jobs corresponding to downloading individual databases.
    Each pod has a mounted this \texttt{PVC}.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{images/proteinconformationprediction_controller}
...
\begin{enumerate}
    ...
    The \emph{AlphaFold} program expects initial settings (for example sequence amino acids) as a JSON file named \texttt{fold\_input.json}.
...
\begin{itemize}
    \item configuring the \texttt{fold\_input.json} file for the AlphaFold program,
...
To save the \texttt{fold\_input.json} file, the \texttt{ENCODED\_INPUT} variable should be set, which should contain the encoded in \texttt{base64} JSON configuration.

To send artifacts, the \texttt{BUCKET} environment variable should be set to the S3 bucket path (full URL, including \texttt{s3:\/\/}).
